...
must_haves:
  truths:
    ...
    - "internal/notification package compiles (pagination assertion fixed)"
...
<task type="auto">
  <name>Task 3: Fix internal/store/postgres pgx v4 imports and add sqlmock</name>
...
**Add sqlmock dependency for internal/store/postgres tests:**

The internal/store/postgres tests use DATA-DOG/go-sqlmock extensively. These tests use database/sql patterns through pgx's stdlib compatibility layer. A full migration to pgxmock would require significant rewrite and is out of scope.

**Decision: Add sqlmock as a dev dependency for these tests.**
...
1. These tests use pgx's stdlib adapter (database/sql interface)
2. sqlmock tests the SQL layer, pgxmock tests pgx-native layer
3. Both can coexist - they test different interfaces
4. A full rewrite to pgxmock is a separate cleanup task

**If sqlmock still doesn't work after import fixes:**
- Check if tests use `pgx.Connect()` vs `sql.Open()`
- If using pgx-native, rewrite to pgxmock
- If using stdlib, sqlmock should work
  </action>
  <verify>
```bash
...
First should return nothing (or just the files that need sqlmock).
  ...
  <done>pgx/v4 imports updated to v5 in internal/store/postgres test files; sqlmock added for stdlib-based tests</done>
</task>
...
- [ ] sqlmock added for stdlib-based postgres tests
