...
<objective>
Fix remaining interface mismatches and type errors in handlers, models, and internal/auth packages.

Purpose: Resolve MockTripModel, MockUserStore interface gaps; fix jwt.Parser.Parts API change; fix undefined types.
Output: handlers, models/user/service, models/trip/service, and internal/auth packages compile.
</objective>
...
3. **internal/auth compiles (jwt.Parser.Parts fix deferred to Task if needed):**
   ```bash
   go test -c ./internal/auth/... 2>&1 | grep "FAIL"
   ```
   Returns nothing or only non-type errors.
</verification>
