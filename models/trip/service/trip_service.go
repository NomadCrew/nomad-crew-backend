	...
	if s.supabaseService != nil && s.supabaseService.IsEnabled() {
		...
		go func() {
			...
			if trip.CreatedBy != nil && *trip.CreatedBy != "" {
				...
				if user, err := s.userStore.GetUserByID(syncCtx, *trip.CreatedBy); err == nil && user != nil && user.ID != "" {
					...
					if err := s.supabaseService.SyncUser(syncCtx, userSyncData); err != nil {
						...
						log.Errorw("Failed to sync user to Supabase before trip sync", "error", err, "userID", *trip.CreatedBy, "supabaseID", user.ID)
						// Continue with trip sync even if user sync fails
					} else {
						log := logger.GetLogger()
						log.Infow("Successfully synced user to Supabase before trip sync", "userID", *trip.CreatedBy, "supabaseID", user.ID)
					}
				} else {
					log := logger.GetLogger()
					if err != nil {
						...
						log.Warnw("User not found for trip sync", "userID", *trip.CreatedBy)
					} else {
						log.Warnw("User has no Supabase ID for trip sync", "userID", *trip.CreatedBy)
					}
					log.Infow("Skipping trip sync to Supabase due to missing or invalid user data", "tripID", trip.ID)
				}
			} else {
				log := logger.GetLogger()
				log.Infow("Trip has no creator, skipping sync to Supabase", "tripID", trip.ID)
			}

			// Only sync if we have a valid Supabase user ID
			if shouldSync {
	...
	if shouldSync {
		...
		go func() {
			...
			if updatedTrip.CreatedBy != nil && *updatedTrip.CreatedBy != "" {
				// Get user data from local store to get the Supabase ID
				if user, err := s.userStore.GetUserByID(syncCtx, *updatedTrip.CreatedBy); err == nil && user != nil && user.ID != "" {
					createdBySupabaseID = user.ID
					canSync = true
					...
					if err := s.supabaseService.SyncUser(syncCtx, userSyncData); err != nil {
						log := logger.GetLogger()
						log.Errorw("Failed to sync user to Supabase before trip update sync", "error", err, "userID", *updatedTrip.CreatedBy, "supabaseID", user.ID)
						// Continue with trip sync even if user sync fails
					}
				} else {
					log := logger.GetLogger()
					if err != nil {
						...
						log.Warnw("User not found for trip update sync", "userID", *updatedTrip.CreatedBy)
					} else {
						log.Warnw("User has no Supabase ID for trip update sync", "userID", *updatedTrip.CreatedBy)
					}
					log.Infow("Skipping trip update sync to Supabase due to missing or invalid user data", "tripID", id)
				}
			} else {
				log := logger.GetLogger()
				log.Infow("Trip has no creator, skipping update sync to Supabase", "tripID", id)
			}
