...
func (s *EventService) cleanupStaleConnections() {
	...
	for tripID, subs := range s.subscribers {
		for subID, sub := range subs {
			// New: Check for duplicate user connections in the same trip
			if existingSub, exists := s.findDuplicateUserConnection(tripID, sub.userID, subID); exists {
				close(existingSub.channel)
				delete(subs, subID)
				s.userConnections[sub.userID]--
			}

			// Existing stale connection check
			if sub.lastActivity.Before(now.Add(-2 * s.config.HeartbeatInterval)) {
...
// New helper function
func (s *EventService) findDuplicateUserConnection(tripID, userID, currentSubID string) (*subscriber, bool) {
	if subs, exists := s.subscribers[tripID]; exists {
		for subID, sub := range subs {
			if sub.userID == userID && subID != currentSubID {
				return sub, true
			}
		}
	}
...
func (s *EventService) Subscribe(ctx context.Context, tripID string, userID string) (<-chan types.Event, error) {
	...
	// New: Check for existing connections for this user in the trip
	if tripSubs, exists := s.subscribers[tripID]; exists {
		for subID, sub := range tripSubs {
			if sub.userID == userID {
				// Close existing connection
				close(sub.channel)
				delete(tripSubs, subID)
	...
	// Existing connection limit checks
