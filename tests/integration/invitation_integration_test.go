...
func TestMain(m *testing.M) {
	if runtime.GOOS == "windows" {
		log.Println("Skipping integration tests on Windows due to Docker limitations.")
		os.Exit(0)
	}
	...
	defer func() {
		if err := postgresContainer.Terminate(ctx); err != nil {
			logOutput.Fatalf("Could not stop postgres container: %s", err)
		}
	}()
...
func setupTestRouterAndDeps(t *testing.T) *gin.Engine {
	...
	userService := user_service.NewUserService(userStore) // Corrected: only userStore
...
func TestDeclineInvitation_Success_InviteeIDKnown(t *testing.T) {
	// Ensure router and DB are ready
	testRouter := setupTestRouterAndDeps(t)
	ctx := context.Background()
	...
	inviterUser, err := createUserInDB(ctx, testDBPool, inviterEmail, inviterSupabaseID, "Inviter User")
	require.NoError(t, err)
	...
	inviteeUser, err := createUserInDB(ctx, testDBPool, inviteeEmail, inviteeSupabaseID, "Invitee User")
	require.NoError(t, err)
	require.NotNil(t, inviteeUser)

	// Create trip
	trip, err := createTripInDB(ctx, testDBPool, "Trip for Decline Known ID", inviterUser.ID)
	require.NoError(t, err)
	...
	invitation, err := createInvitationInDB(ctx, testDBPool, trip.ID, inviterUser.ID, actualInviteeIDForDB, inviteeEmail, types.MemberRoleMember, types.InvitationStatusPending, time.Now().Add(24*time.Hour), actualTokenForDB)
	require.NoError(t, err)
	...
	if !assert.Equal(t, http.StatusNoContent, rr.Code, "Expected StatusNoContent, got %d, body: %s", rr.Code, rr.Body.String()) {
		...
		updatedInv, _ := getInvitationFromDB(ctx, testDBPool, invitation.ID)
		if updatedInv != nil {
	...
	updatedInvitation, err := getInvitationFromDB(ctx, testDBPool, invitation.ID)
	require.NoError(t, err)
...
func createInvitationInDB(ctx context.Context, pool *pgxpool.Pool, tripID, inviterID string, inviteeID *string, inviteeEmail string, role types.MemberRole, status types.InvitationStatus, expiresAt time.Time, token *string) (*types.TripInvitation, error) {
	...
	query := `
		INSERT INTO trip_invitations (id, trip_id, inviter_id, invitee_id, invitee_email, role, status, expires_at, token, created_at, updated_at)
		VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9, NOW(), NOW())
		RETURNING id, trip_id, inviter_id, invitee_id, invitee_email, role, status, created_at, updated_at, expires_at, token
	`
	var inv types.TripInvitation
	err := pool.QueryRow(ctx, query, invID, tripID, inviterID, inviteeID, inviteeEmail, role, status, expiresAt, token).Scan(
		&inv.ID, &inv.TripID, &inv.InviterID, &inv.InviteeID, &inv.InviteeEmail, &inv.Role, &inv.Status, &inv.CreatedAt, &inv.UpdatedAt, &inv.ExpiresAt, &inv.Token,
	)
	if err != nil {
...
func getInvitationFromDB(ctx context.Context, pool *pgxpool.Pool, invitationID string) (*types.TripInvitation, error) {
	query := `
		SELECT id, trip_id, inviter_id, invitee_id, invitee_email, role, status, created_at, updated_at, expires_at, token
