...
func createTripInDB(ctx context.Context, pool *pgxpool.Pool, name, ownerID string) (*types.Trip, error) {
	...
	// The ID is generated by the database (DEFAULT uuid_generate_v4()), so we don't pre-generate.
	// We will retrieve it using RETURNING.
	query := `
		INSERT INTO trips (name, created_by, start_date, end_date, status, description, destination_address)
		VALUES ($1, $2, $3, $4, $5, $6, $7)
		RETURNING id, name, created_by, start_date, end_date, status, description, created_at, updated_at, 
		          destination_address, background_image_url
	` // Removed destination_place_id from RETURNING

	now := time.Now()
	trip := &types.Trip{}
	var destAddress sql.NullString
	// var destPlaceID sql.NullString // Removed
	var bgImageURL sql.NullString

	err := pool.QueryRow(ctx, query,
		// tripID, // Removed pre-generated ID
		name,
		ownerID,
		now,                           // start_date
	...
	// trip.Destination.PlaceID = destPlaceID.String // Removed
