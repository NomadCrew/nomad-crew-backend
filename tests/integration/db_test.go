...
func TestTripDB_Integration(t *testing.T) {
	...
	defer func() {
		t.Run("Cleanup", func(t *testing.T) {
			...
			_, err := dbClient.GetPool().Exec(ctx, `DELETE FROM metadata`)
			require.NoError(t, err)

			// Then clean up main tables in correct order to handle foreign keys
			_, err = dbClient.GetPool().Exec(ctx, `
